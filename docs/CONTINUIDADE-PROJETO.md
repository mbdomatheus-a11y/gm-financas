...
- **26/09/2026 — banco oficial estava atrasado, agora sincronizado**: encontramos 18 migrações já commitadas (19/09 a 25/09/2026) que nunca tinham sido aplicadas no Supabase oficial `wjapagkdgjlavonbmjdu`. Todas foram aplicadas em ordem cronológica nesta data, incluindo `20260919100000_administracao_privacidade_modulos.sql`, `20260919110000_comercial_pet_exames_alertas.sql`, `20260919120000_identidade_visual_site.sql`, `20260919121000_receitas_por_cartao.sql`, `20260920010000_admin_uso_arquivos.sql`, `20260920020000_recuperacao_conta_email.sql`, `20260920021000_bloqueio_conta_inativa.sql`, `20260922010000_configuracoes_acesso_modulos_grupos.sql`, `20260923010000_totais_fatura_limites_versoes.sql`, `20260923020000_fix_despesas_origem_check.sql` e `20260925190000_admin_abas_privacidade_chamados.sql`, entre outras. `types.ts` foi regenerado e `tsc`/`build` validados após a atualização. Ver `docs/especificacoes-e-seguranca.md`, revisão de 26/09/2026, para o detalhe completo e os módulos afetados.
- `supabase/migrations/20260926010000_rls_restringe_exclusao_por_permissao.sql`: cria `private.pode_excluir(modulo)` e policies RESTRICTIVE de `DELETE` nas 6 tabelas financeiras, espelhando no banco a regra de `permissoes.pode_excluir` que já existia só na interface.
- `supabase/migrations/20260926020000_bloqueio_exclusao_total_por_conta.sql`: cria `profiles.bloqueio_exclusao_total`, `private.bloqueio_exclusao_total()` e policies RESTRICTIVE de `DELETE` em 13 tabelas, corrigindo o bloqueio da conta compartilhada (Guilherme) que antes comparava com um CPF fixo e nunca funcionava. `useAuthData.ts` (`usePermissoes`) foi atualizado para ler essa coluna.
...
- **Verificar sempre se `supabase/migrations` está de fato aplicado no projeto oficial antes de assumir que uma feature funciona em produção.** Em 26/09/2026 encontramos 18 arquivos já commitados havia dias que nunca tinham sido aplicados — `mcp__Supabase__list_migrations` não é uma fonte confiável para decidir isso (a numeração rastreada não corresponde 1:1 aos nomes de arquivo do repositório); confirme empiricamente via `execute_sql`/`generate_typescript_types` contra o schema real.
- O bloqueio de exclusão por conta (`profiles.bloqueio_exclusao_total`) e por módulo (`permissoes.pode_excluir`) agora também são reforçados no banco via policies RESTRICTIVE de `DELETE`. Qualquer nova tabela sensível adicionada ao produto deve considerar se precisa do mesmo reforço, não apenas da checagem em `usePermissoes()`.
